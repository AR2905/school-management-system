...
import { AnnouncementSchema, announcementSchema, customZodResolver } from "@/lib/formValidationSchemas";
...
const AnnouncementForm = ({
  ...
  const {
    ...
    resolver: customZodResolver(announcementSchema),
  });
      ...
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          >
            ...
            {relatedData.classes.map((className: { id: number; name: string }) => (
              <option value={className.id} key={className.id}>
                {className.id} : {className.name}
              </option>
            ))}
          </select>
        </div>
